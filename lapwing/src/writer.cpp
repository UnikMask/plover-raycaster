...
uintptr_t Writer::writeImage(Entry &content, std::string path) {
    ...
    if (fileExists(path)) {
        u8* image = stbi_load(path.c_str(), &textureMetadata.width, &textureMetadata.height, &textureMetadata.bitDepth, 0);
        assets->write((char*)&textureMetadata, sizeof(TextureMetadata));
        assets->write((char*)image, textureMetadata.width * textureMetadata.height * textureMetadata.bitDepth);
        stbi_image_free(image);
        content.size = sizeof(TextureMetadata) + textureMetadata.width * textureMetadata.height * textureMetadata.bitDepth;
...
uintptr_t Writer::writeModel(Entry &content, std::string path) {
    ...
    if (fileExists(path)) {
        ...
        assets->write((char*)vertices.data(), modelMetadata.vertexCount * sizeof(Vertex));
        assets->write((char*)indices.data(), modelMetadata.indexCount * sizeof(uint32_t));

        content.size = sizeof(ModelMetadata) + modelMetadata.vertexCount * sizeof(Vertex) + modelMetadata.indexCount * sizeof(uint32_t);
        return content.offset + content.size;
    } else {
        std::cerr << "Asset file " << path << " does not exist." << std::endl;
    }
