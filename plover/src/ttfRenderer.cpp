...
void drawGlyphs(u32 x, u32 y, char *text, Bitmap& bitmap) {
	...
	u32 pen_x = x, pen_y = face->ascender / FONT_SCALE + y;
	int error;
...
const u32 GLYPH_BIAS = 4;

void buildGlyphAtlas(VulkanContext *context, GlyphAtlas *atlas) {
	FT_GlyphSlot slot = face->glyph;
	int error;

	atlas->charQuadWidth = (face->bbox.xMax - face->bbox.xMin) / FONT_SCALE + GLYPH_BIAS;
	atlas->charQuadHeight = (face->bbox.yMax - face->bbox.yMin) / FONT_SCALE;

	// TODO(oliver): use an arena to do the bitmap alloc
	Bitmap bitmaps[95];
	// NOTE(oliver): loop over all printable ASCII chars
	for (char c = MIN_PRINTABLE_CHAR; c <= MAX_PRINTABLE_CHAR; c++) {
		error = FT_Load_Char(face, c, FT_LOAD_RENDER);
		assert(!error && "Failed to rasterize glyph!");

		CharData *cData = &(atlas->chars[c - MIN_PRINTABLE_CHAR]);
		cData->xAdvance = (slot->advance.x >> 6);
		createBitmap(&bitmaps[c - MIN_PRINTABLE_CHAR],
					 atlas->charQuadWidth,
					 atlas->charQuadHeight,
					 RGBA8);


		// NOTE(oliver): Draw glyphs aligned to baseline, means we don't need to do anything fancy later
		drawGlyph(bitmaps[c - MIN_PRINTABLE_CHAR],
				  slot->bitmap,
				  slot->bitmap_left + GLYPH_BIAS,
				  face->bbox.yMax / FONT_SCALE - slot->bitmap_top );

		i32 glyphIdx = FT_Get_Char_Index(face, c);
		i32 kernGlyphIdx, xKerningDesign, xKerning;
		for (char kernChar = MIN_PRINTABLE_CHAR;
			 kernChar < MAX_PRINTABLE_CHAR;
			 kernChar++) {
			kernGlyphIdx = FT_Get_Char_Index(face, kernChar);
			xKerningDesign = getGlyphKerning(glyphIdx, kernGlyphIdx);
			xKerning = (xKerningDesign / (f32) face->units_per_EM) * (FONT_SIZE / 72.0) * 96; // NOTE(oliver): Formula per OpenType spec
			if (xKerning != 0) {
				KernData data{};
				data.character = kernChar;
				data.kerning_x = xKerning;
				cData->kernData.push_back(data);
			}
		}
	}

	ArrayTextureCreateInfo arrayTextureCreateInfo{};
	arrayTextureCreateInfo.bitmaps = bitmaps;
	arrayTextureCreateInfo.layers = 95;
	
	createArrayTexture(context, arrayTextureCreateInfo, &atlas->texture);

	for (char ch = MIN_PRINTABLE_CHAR; ch <= MAX_PRINTABLE_CHAR; ch++) {
		delete[] (u8*) bitmaps[ch - MIN_PRINTABLE_CHAR].pixels;
	}
}
